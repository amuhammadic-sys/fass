...
private void DrawInGameBoosterSettings()
{
    ...
    foreach (var booster in Enum.GetValues(typeof(BoosterType)))
    {
        ...
        var boosterType = (BoosterType)booster;

        // FIX: prevent KeyNotFoundException
        if (!availableBoosters.ContainsKey(boosterType))
            availableBoosters[boosterType] = false;

        availableBoosters[boosterType] =
            EditorGUILayout.Toggle(availableBoosters[boosterType], GUILayout.Width(30));
